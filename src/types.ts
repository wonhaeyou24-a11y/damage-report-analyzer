...
// 비탈면(상부자연사면~보수시설)과 옹벽(전면부~지반및기초부) 부위값이 한 목록에 같이 있다 —
// 실제 옹벽 보고서 2건을 분석해보니(2026-09) 옹벽 손상 대부분이 기존 목록(비탈면 전용)에
// 맞는 분류가 없어 전부 "기타"로 밀려나는 문제가 있었다. 시설물 유형마다 별도 목록을 두지
// 않고 하나로 합쳐둔 이유는, part가 보고서에서 실제로 쓰는 표현을 최대한 존중해 AI가 골라야
// 하는 값이지 시설물 유형별로 프로그램이 미리 강제할 값이 아니기 때문이다(범용성 원칙).
